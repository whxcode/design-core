...
// 在文件末尾或开头添加一个空函数
extern "C" {
void force_link_wasmapi() {
}
}

using namespace emscripten;

// 在 wasmapi/src/Binding.cpp 顶部定义一个辅助函数
...
EMSCRIPTEN_BINDINGS(core_api) {
    ...
    class_<ZApp>("App")
        ...
        .function("exportDocument", optional_override([](ZApp& self) -> val {
                      return ZWasmDocumentAdapter::ToValue(self.exportDocumentPackage());
                  }))

        .function("loadDocument", optional_override([](ZApp& self, val payload) -> val {
                      auto documentPayload = ZWasmDocumentAdapter::ToDocumentPayload(payload);
                      if (!documentPayload.success) {
                          ZDocumentIOResult result;
                          result.message = documentPayload.message;
                          return ZWasmDocumentAdapter::ToValue(result);
                      }

                      auto result = documentPayload.legacy
                                        ? self.openLegacyDocument(documentPayload.legacyDocument)
                                        : self.openDocumentPackage(documentPayload.package);
                      return ZWasmDocumentAdapter::ToValue(result);
                  }));
