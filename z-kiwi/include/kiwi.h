...
namespace kiwi {
inline void delete_kiwi_proc(const void* ptr, void* context) {
    int* data = (int*)ptr;
    delete[] data;
}
...
public:
    ...
    void set(const std::initializer_list<T>& data) {
        set(data.begin(), data.size());
    }
...
kiwi::ByteBuffer::ByteBuffer()
    : _data(new uint8_t[INITIAL_CAPACITY]), _capacity(INITIAL_CAPACITY), _ownsData(true) {
}

kiwi::ByteBuffer::ByteBuffer(uint8_t* data, size_t size)
...
uint8_t* kiwi::ByteBuffer::transfer() {
    _ownsData = false;
    return _data;
}
...
void kiwi::ByteBuffer::writeVarFloat(float value) {
    ...
    // Optimization: use a single byte to store zero and denormals (check for an
    // exponent of 0)
    if ((bits & 255) == 0) {
        writeByte(0);
        return;
    ...
    for (auto& definition : _definitions) {
        ...
        for (auto& field : definition.fields) {
            if (!bb.readString(field.name, _pool) || !bb.readVarInt(field.type) ||
                !bb.readByte(field.isArray) || !bb.readVarUint(field.value) ||
                field.type < TYPE_UINT64 || field.type >= (int32_t)definitionCount) {
                return false;
            }
        }
    }
